...
namespace SE456
{
    ...
    abstract public class ManBase
    {
        ...
        public void addToFront(NodeBase _pNodeBase)
        {
            poActive.AddToFront(_pNodeBase);
        }

        public void addBasedOnPriority(NodeBase _pNodeBase)
        {
            poActive.AddBasedOnPriority(_pNodeBase);
        }
        ...
        public void baseRemoveAll()
        {
            ...
            // iterate through the nodes
            pNode = (NodeBase)pIt.Current();

            while (!pIt.IsDone())
            {
                pTmp = pNode;
                pNode = (NodeBase)pIt.Next();
                baseRemove(pTmp);
            }
        }


        public NodeBase baseGetFromReserve()
        {
            Iterator pIt = poReserve.GetIterator();
            Debug.Assert(pIt != null);
        ...
        public void baseRemoveForReorder(NodeBase pNode)
        {
            Debug.Assert(pNode != null);
